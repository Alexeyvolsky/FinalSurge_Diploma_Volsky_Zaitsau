...
public class PalladinoModal extends BaseModal {
    ...
    protected final static String GREEN_BOX = "//h4[text()='%s']";
    ...
    public boolean isGreenBoxIntervalPowerDisplay()  {
        log.debug("verifying that green box in IntervalPower is display");
        try {
    ...
    public boolean isGreenBoxMoreThenFortyDisplay()  {
        log.debug("verifying that green box in MoreThenForty calculator is display");
        try {
    ...
    public boolean isGreenBoxLessThenFortyDisplay()  {
        log.debug("verifying that green box in LessThenForty calculator is display");
        try {
            driver.findElement(By.xpath(String.format(GREEN_BOX, "Estimate Results"))).isDisplayed();
        } catch (NoSuchElementException ex) {
            return false;
    ...
    public boolean isGreenBoxCpAndRwcDisplay()  {
        log.debug("verifying that green box in CpAndRwc calculator is display");
        try {
            driver.findElement(By.xpath(String.format(GREEN_BOX, "Estimate Results"))).isDisplayed();
        } catch (NoSuchElementException ex) {
            return false;
